...
Java_com_danzku_imagetoeps_MainActivity_nativeToEps(
    ...
    std::ofstream eps(result,std::ios::out|std::ios::trunc);
    ...
    int w=width,h=height,step=std::max(1,(int)cell);
    double scale=1000.0/std::max(w,h);

    eps<<"%!PS-Adobe-3.0 EPSF-3.0\n";
    eps<<"%%Creator: ImageToEPS\n";
    eps<<"%%BoundingBox: 0 0 "<<(int)std::ceil(w*scale)<<" "
       <<(int)std::ceil(h*scale)<<"\n";
    eps<<"%%LanguageLevel: 2\n%%Pages: 1\n%%EndComments\n";
    eps<<"/r { newpath 4 2 roll moveto dup 0 rlineto exch 0 exch rlineto "
    ...
    for(int y=0;y<h;y+=step) for(int x=0;x<w;x+=step) {
        ...
        for(int yy=y;yy<y2;yy++) for(int xx=x;xx<x2;xx++) {
            sum+=gray(px[yy*w+xx]); count++;
        }
        if(count && sum/count<threshold) {
            double X=x*scale, Y=(h-y2)*scale;
            double W=(x2-x)*scale, H=(y2-y)*scale;
            eps<<X<<" "<<Y<<" "<<W<<" "<<H<<" r\n";
        }
    }
    eps<<"showpage\n%%EOF\n";
